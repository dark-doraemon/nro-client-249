...
public class GameScr : mScreen, IChatable
{
	...
	public bool isAttack()
	{
		...
		if (Char.myCharz().charFocus != null)
		{
			...
			if (Char.myCharz().isMeCanAttackOtherPlayer(Char.myCharz().charFocus) || Char.myCharz().isSelectingSkillBuffToPlayer())
			{
				...
				if (num13 <= Char.myCharz().myskill.dx && num14 <= Char.myCharz().myskill.dy)
				{
					...
					if ((TileMap.tileTypeAtPixel(Char.myCharz().cx, Char.myCharz().cy + 3) & 2) == 2)
					{
						...
						if ((TileMap.tileTypeAtPixel(Char.myCharz().charFocus.cx + num15 * num16, Char.myCharz().cy + 3) & 2) != 2)
						{
							flag6 = true;
						}
					}
					if (num13 <= num15 && !flag6)
					{
						if (Char.myCharz().cx > Char.myCharz().charFocus.cx)
						{
							Char.myCharz().cx = Char.myCharz().charFocus.cx + num15;
							Char.myCharz().cdir = -1;
						}
						else
						{
				...
				int num17 = (Char.myCharz().myskill.dx - 20) * ((Char.myCharz().cx > Char.myCharz().charFocus.cx) ? 1 : (-1));
				if (num13 <= Char.myCharz().myskill.dx)
				{
					num17 = 0;
				}
				Char.myCharz().currentMovePoint = new MovePoint(Char.myCharz().charFocus.cx + num17, Char.myCharz().charFocus.cy);
				Char.myCharz().endMovePointCommand = new Command(null, null, 8002, null);
				GameCanvas.clearKeyHold();
				GameCanvas.clearKeyPressed();
				return false;
			...
			{
				int num18 = (20 + Res.r.nextInt(20)) * ((Char.myCharz().cx > Char.myCharz().charFocus.cx) ? 1 : (-1));
	...
	public override void paint(mGraphics g)
	{
		...
		if (TileMap.mapID == 120)
		{
			if (percentMabu != 100)
			{
				...
				sbyte percentMabu2 = percentMabu;
		...
		if (!isPaintOther)
		{
			...
			for (int num17 = 0; num17 < vCharInMap.size(); num17++)
			{
				...
				try
				{
					obj6 = (Char)vCharInMap.elementAt(num17);
				}
				catch (Exception)
				{
				}
			...
			if (Char.myCharz().clan != null)
			{
				...
				{
					Char obj7 = (Char)vCharInMap.elementAt(num22);
					...
					if (obj7.isOutX() && obj7.cx < Char.myCharz().cx)
					{
						...
						if (num19 <= num23)
						{
							mFont.tahoma_7_green.drawString(g, obj7.cName, 20, num18 - 12 + num19 * 12, mFont.LEFT, mFont.tahoma_7_grey);
							obj7.paintHp(g, 10, num18 + num19 * 12 - 5);
							num19++;
						}
					}
					else if (obj7.isOutX() && obj7.cx > Char.myCharz().cx && num20 <= num21)
					{
						mFont.tahoma_7_green.drawString(g, obj7.cName, GameCanvas.w - 25, num18 - 12 + num20 * 12, mFont.RIGHT, mFont.tahoma_7_grey);
						obj7.paintHp(g, GameCanvas.w - 15, num18 + num20 * 12 - 5);
						num20++;
					}
				}
			}
	...
	private void paintSelectedSkill(mGraphics g)
	{
		...
		else
		{
			...
			if (isHaveSelectSkill)
			{
				...
				int keyTouch2 = mScreen.keyTouch;
				int num6 = 10;
				if (!GameCanvas.isTouch)
				{
					g.setColor(11152401);
	...
	public void actionPerform(int idAction, object p)
	{
		...
		case 11113:
		{
			...
			if (obj4 != null)
			{
				Service.gI().giaodich(0, obj4.charID, -1, -1);
			}
			break;
		}
		case 11114:
		{
			...
			if (obj5 != null)
			{
				Service.gI().giaodich(1, obj5.charID, -1, -1);
			}
			break;
		}
		case 11111:
		...
		case 11120:
		{
			object[] obj3 = (object[])p;
			Skill skill4 = (Skill)obj3[0];
			int num2 = int.Parse((string)obj3[1]);
		...
		case 11121:
		{
			...
			int num = int.Parse((string)obj2[1]);
			for (int i = 0; i < keySkill.Length; i++)
			{
	...
	public void paint_CT(mGraphics g, int x, int y, int w)
	{
		...
		int frameHeight = fra_PVE_Bar_0.frameHeight;
		int num11 = mGraphics.getImageHeight(imgBall) / 2;
		int frameWidth = fra_PVE_Bar_1.frameWidth;
		...
		for (int i = 0; i < num8; i++)
		{
			if (i < num8 - 1)
			{
				g.drawRegion(img_ct_bar_0, 0, 15, mGraphics.getImageWidth(img_ct_bar_0), 15, 2, num4 + fra_PVE_Bar_0.frameWidth + i * fra_PVE_Bar_0.frameWidth, num6, mGraphics.TOP | mGraphics.LEFT, true);
			}
			else
			{
				g.drawRegion(img_ct_bar_0, 0, 15, mGraphics.getImageWidth(img_ct_bar_0), 15, 2, num4 + num7, num6, mGraphics.TOP | mGraphics.LEFT, true);
			}
			if (i < num8 - 1)
			{
				g.drawRegion(img_ct_bar_0, 0, 15, mGraphics.getImageWidth(img_ct_bar_0), 15, 2, num5 + i * fra_PVE_Bar_0.frameWidth, num6, mGraphics.TOP | mGraphics.LEFT, true);
			}
			else
			{
				g.drawRegion(img_ct_bar_0, 0, 15, mGraphics.getImageWidth(img_ct_bar_0), 15, 2, num5 + num7 - fra_PVE_Bar_0.frameWidth, num6, mGraphics.TOP | mGraphics.LEFT, true);
			}
		}
		...
		if (nCT_TeamA > 0)
		{
			for (int j = 0; j < num2; j++)
			{
				if (j == 0)
				{
					g.drawRegion(img_ct_bar_0, 0, 60, mGraphics.getImageWidth(img_ct_bar_0), 15, 2, num4, num6, mGraphics.TOP | mGraphics.LEFT, true);
				}
				else
				{
					g.drawRegion(img_ct_bar_0, 0, 75, mGraphics.getImageWidth(img_ct_bar_0), 15, 2, num4 + j * 6, num6, mGraphics.TOP | mGraphics.LEFT, true);
				}
			}
		}
		GameCanvas.resetTrans(g);
		...
		if (nCT_TeamB > 0)
		{
			for (int k = 0; k < num2; k++)
			{
				if (k == 0)
				{
					g.drawRegion(img_ct_bar_0, 0, 30, mGraphics.getImageWidth(img_ct_bar_0), 15, 0, num5 + num7, num6, mGraphics.TOP | mGraphics.LEFT, true);
				}
				else
				{
					g.drawRegion(img_ct_bar_0, 0, 45, mGraphics.getImageWidth(img_ct_bar_0), 15, 0, num5 + num7 - k * 6, num6, mGraphics.TOP | mGraphics.LEFT, true);
				}
			}
		}
		GameCanvas.resetTrans(g);
