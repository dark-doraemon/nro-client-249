...
public class Panel : IActionListener, IChatable
{
	...
	private void updateKeyInTabBar()
	{
		...
		if (isTabInven() && isnewInventory)
		{
			...
			int sellectInventory2 = sellectInventory;
			int num2 = 0;
	...
	private void paintGameSubInfo(mGraphics g)
	{
		...
		{
			int xScroll2 = xScroll;
			int num = yScroll + i * 15;
			int wScroll2 = wScroll;
	...
	private void paintMapTrans(mGraphics g)
	{
		...
		for (int i = 0; i < mapNames.Length; i++)
		{
			...
			int wScroll2 = wScroll;
			int h = ITEM_HEIGHT - 1;
			int xScroll3 = xScroll;
			int yScroll2 = yScroll;
			int iTEM_HEIGHT = ITEM_HEIGHT;
			int iTEM_HEIGHT2 = ITEM_HEIGHT;
			if (num - cmy <= yScroll + hScroll && num - cmy >= yScroll - ITEM_HEIGHT)
			{
				g.setColor((i != selected) ? 15196114 : 16383818);
	...
	private void doFireSkill()
	{
		...
		if (selected == 0 || selected == 1 || selected == 2 || selected == 3 || selected == 4 || selected == 5)
		{
			...
			if (selected == 0)
			{
				if (cTiemNang < Char.myCharz().cHPGoc + num)
				{
					GameCanvas.startOKDlg(mResources.not_enough_potential_point1 + Char.myCharz().cTiemNang + mResources.not_enough_potential_point2 + (Char.myCharz().cHPGoc + num), false);
					return;
				}
				if (cTiemNang > cHPGoc && cTiemNang < 10 * (2 * (cHPGoc + num) + 180) / 2)
				{
					GameCanvas.startYesNoDlg(mResources.use_potential_point_for1 + (cHPGoc + num) + mResources.use_potential_point_for2 + Char.myCharz().hpFrom1000TiemNang + mResources.for_HP, new Command(mResources.increase_upper, this, 9000, null), new Command(mResources.CANCEL, this, 4007, null));
					return;
				}
				if (cTiemNang >= 10 * (2 * (cHPGoc + num) + 180) / 2 && cTiemNang < 100 * (2 * (cHPGoc + num) + 1980) / 2)
				{
					MyVector myVector = new MyVector(string.Empty);
					myVector.addElement(new Command(mResources.increase_upper + "\n" + Char.myCharz().hpFrom1000TiemNang + mResources.HP + "\n-" + Res.formatNumber2(cHPGoc + num), this, 9000, null));
					myVector.addElement(new Command(mResources.increase_upper + "\n" + 10 * Char.myCharz().hpFrom1000TiemNang + mResources.HP + "\n-" + Res.formatNumber2(10 * (2 * (cHPGoc + num) + 180) / 2), this, 9006, null));
					GameCanvas.menu.startAt(myVector, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
					addSkillDetail2(selected);
				}
				if (cTiemNang >= 100 * (2 * (cHPGoc + num) + 1980) / 2)
				{
					MyVector myVector2 = new MyVector(string.Empty);
					myVector2.addElement(new Command(mResources.increase_upper + "\n" + Char.myCharz().hpFrom1000TiemNang + mResources.HP + "\n-" + Res.formatNumber2(cHPGoc + num), this, 9000, null));
					myVector2.addElement(new Command(mResources.increase_upper + "\n" + 10 * Char.myCharz().hpFrom1000TiemNang + mResources.HP + "\n-" + Res.formatNumber2(10 * (2 * (cHPGoc + num) + 180) / 2), this, 9006, null));
					myVector2.addElement(new Command(mResources.increase_upper + "\n" + 100 * Char.myCharz().hpFrom1000TiemNang + mResources.HP + "\n-" + Res.formatNumber2(100 * (2 * (cHPGoc + num) + 1980) / 2), this, 9007, null));
					GameCanvas.menu.startAt(myVector2, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
					addSkillDetail2(selected);
				}
			}
			if (selected == 1)
			{
				if (Char.myCharz().cTiemNang < Char.myCharz().cMPGoc + num)
				{
					GameCanvas.startOKDlg(mResources.not_enough_potential_point1 + Char.myCharz().cTiemNang + mResources.not_enough_potential_point2 + (Char.myCharz().cMPGoc + num));
					return;
				}
				if (cTiemNang > cMPGoc && cTiemNang < 10 * (2 * (cMPGoc + num) + 180) / 2)
				{
					GameCanvas.startYesNoDlg(mResources.use_potential_point_for1 + (cMPGoc + num) + mResources.use_potential_point_for2 + Char.myCharz().mpFrom1000TiemNang + mResources.for_KI, new Command(mResources.increase_upper, this, 9000, null), new Command(mResources.CANCEL, this, 4007, null));
					return;
				}
				...
				if (cTiemNang >= 100 * (2 * (cMPGoc + num) + 1980) / 2)
				{
					MyVector myVector4 = new MyVector(string.Empty);
					myVector4.addElement(new Command(mResources.increase_upper + "\n" + Char.myCharz().mpFrom1000TiemNang + mResources.KI + "\n-" + Res.formatNumber2(cMPGoc + num), this, 9000, null));
					myVector4.addElement(new Command(mResources.increase_upper + "\n" + 10 * Char.myCharz().mpFrom1000TiemNang + mResources.KI + "\n-" + Res.formatNumber2(10 * (2 * (cMPGoc + num) + 180) / 2), this, 9006, null));
					myVector4.addElement(new Command(mResources.increase_upper + "\n" + 100 * Char.myCharz().mpFrom1000TiemNang + mResources.KI + "\n-" + Res.formatNumber2(100 * (2 * (cMPGoc + num) + 1980) / 2), this, 9007, null));
					GameCanvas.menu.startAt(myVector4, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
					addSkillDetail2(selected);
				}
			}
			...
			else if (selected == 4)
			{
				...
				num2 = t_tiemnang[num3];
				if (Char.myCharz().cTiemNang < num2)
				{
					GameCanvas.startOKDlg(mResources.not_enough_potential_point1 + Res.formatNumber2(Char.myCharz().cTiemNang) + mResources.not_enough_potential_point2 + Res.formatNumber2(num2));
					return;
				}
				GameCanvas.startYesNoDlg(mResources.use_potential_point_for1 + Res.formatNumber(num2) + mResources.use_potential_point_for2 + Char.myCharz().criticalFrom1000Tiemnang + mResources.for_crit, new Command(mResources.increase_upper, this, 9000, null), new Command(mResources.CANCEL, this, 4007, null));
			}
			else if (selected == 5)
			{
		...
		int num4 = selected - 6;
		SkillTemplate skillTemplate = Char.myCharz().nClass.skillTemplates[num4];
		Skill skill = Char.myCharz().getSkill(skillTemplate);
		Skill skill2 = null;
